...
function GenAI() {
  ...
  // kicks off the conversation between the two chatbots
  useEffect(() => {
    handleSocratesCall(nietzschePersona.introduction);
  }, []);
  ...
  const handleSocratesCall = async (message) => {
    ...
    ++apiCallCounter;
    console.log(
      "Socrates API Call Counter after call && increment: ",
      apiCallCounter
    );
  ...
  const handleNietzscheCall = async (message) => {
    ...
    ++apiCallCounter;
    console.log(
      "Nietzsche API Call Counter after call && increment: ",
      apiCallCounter
    );
